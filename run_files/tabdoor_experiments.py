        ...
        for model in model_list:
            for target_label in target_labels:
                for epsilon in epsilon_list:
                    job_script_file = f"exp_tabdoor_{exp_num}_{dataset}_{model}_{target_label}_{epsilon}.sh"
                    ...
                    # Create the command to run the experiment with proper CPU binding
                    # Use a more compatible approach without explicit CPU binding flags
                    command = f"srun python tabdoor.py --dataset_name {dataset} --model_name {model} --target_label {target_label} --epsilon {epsilon} --exp_num {exp_num}"

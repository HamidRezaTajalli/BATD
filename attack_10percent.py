...
class Attack:
    ...
    def __init__(self, device, model, data_obj, target_label, mu, beta, lambd, epsilon):
        ...
        # Create a new dataset with 10% of the samples from the original training dataset
        original_dataset = self.converted_dataset[0]
        total_samples = len(original_dataset)
        num_samples_10percent = int(0.1 * total_samples)
        
        # Randomly select 10% of the indices
        indices = torch.randperm(total_samples)[:num_samples_10percent]
        
        # Create new tensors with only the selected samples
        X_10percent = torch.stack([original_dataset[i][0] for i in indices])
        y_10percent = torch.stack([original_dataset[i][1] for i in indices])
        
        # Create a new TensorDataset with 10% of the samples
        self.converted_dataset_10percent = TensorDataset(X_10percent, y_10percent)
    ...
    def select_non_target_samples(self) -> TensorDataset:
        ...
        X_train, y_train = self.data_obj._get_dataset_data(self.converted_dataset_10percent)
    ...
    def compute_min_max(self):
        ...
        # Extract training data
        X_train, _ = self.data_obj._get_dataset_data(self.converted_dataset_10percent)
    ...
    def save_trigger(self, filepath=None):
        ...
        if filepath is None:
            ...
            filepath = file_dir / f"{self.data_obj.dataset_name}_{self.model.model_name}_{self.target_label}_{self.mu}_{self.beta}_{self.lambd}_delta_10percent.pt"
    ...
    def save_poisoned_dataset(self, filepath=None):
        ...
        if filepath is None:
            ...
            file_dir.mkdir(parents=True, exist_ok=True)
            # Define the default filepath
            filepath = file_dir / f"{self.data_obj.dataset_name}_{self.model.model_name}_{self.target_label}_{self.mu}_{self.beta}_{self.lambd}_{self.epsilon}_poisoned_dataset_10percent.pt"
